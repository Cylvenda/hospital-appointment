...
export default function ReceptionistAppointmentsPage() {
  ...
  const cancelledAppointments = useMemo(
    () => filterAppointmentsForQueue(appointments, "receptionist", "cancelled"),
    [appointments]
  )
  ...
  return (
    <div className="w-full space-y-4 max-w-8xl">
      <div>
        <h1 className="text-2xl font-semibold">Cancelled Appointments</h1>
        <p className="text-sm text-muted-foreground">
          Review cancelled appointments and manage any required follow-up.
        </p>
      </div>
      ...
      {cancelledAppointments.length === 0 ? (
        <div className="rounded-4xl border border-dashed border-border bg-card p-10 text-center">
          <p className="text-sm text-muted-foreground">
            No cancelled appointments are available.
          </p>
        </div>
      ) : (
