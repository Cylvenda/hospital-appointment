...
export default function ReceptionistAppointmentsPage() {
  ...
  return (
    <div className="w-full space-y-4 max-w-8xl">
      <div>
        <h1 className="text-2xl font-semibold">{t("patientAppointmentLists.cancelledTitle")}</h1>
        <p className="text-sm text-muted-foreground">
          {t("patientAppointmentLists.cancelledDescription")}
        </p>
      </div>
      ...
      {cancelledAppointments.length === 0 ? (
        <div className="rounded-4xl border border-dashed border-border bg-card p-10 text-center">
          <p className="text-sm text-muted-foreground">
            {t("patientAppointmentLists.noCancelled")}
          </p>
        </div>
      ) : (
